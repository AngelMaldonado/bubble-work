...
const en = {
  ...
  'tour.empty.body': 'There is nothing on the board yet. Press <b>⌘K</b> here to create a bubble, then birth a thread into it with a Brief and a Logbook — that is the whole ritual.',
  'mcp.badge': 'MCP',
  ...
  'mcp.prompt':
    'Set up the Bubble Work MCP server in my global MCP configuration.\n\nTransport: streamable HTTP\nURL: {url}\nAuth: send the header  Authorization: Bearer {token}\n\nIf I use Claude Code, the whole thing is one command:\nclaude mcp add --transport http bubble {url} --header \"Authorization: Bearer {token}\"\n\nFor any other client, add an entry to its global config file with type \"http\", that URL, and that Authorization header. Find the right file for the client I actually use rather than guessing at a path.\n\nThen confirm it worked by listing the server tools. You should see list_bubbles, read_thread, birth_thread and update_thread among them. If it fails, tell me the exact error instead of retrying blindly.\n\nTreat the token as a secret: put it in the config file, do not echo it back to me, and do not write it anywhere else.',
...
const es: Record<MsgKey, string> = {
  ...
  'tour.empty.body': 'Todavía no hay nada en el tablero. Presiona <b>⌘K</b> aquí para crear una burbuja, y luego nace un hilo con su Brief y su Bitácora — ése es todo el ritual.',
  'mcp.badge': 'MCP',
  ...
  'mcp.prompt':
    'Configura el servidor MCP de Bubble Work en mi configuración global de MCP.\n\nProtocolo: HTTP (streamable http)\nURL: {url}\nAutenticación: envía la cabecera  Authorization: Bearer {token}\n\nSi uso Claude Code, todo es un solo comando:\nclaude mcp add --transport http bubble {url} --header \"Authorization: Bearer {token}\"\n\nPara cualquier otro cliente, agrega una entrada en su archivo de configuración global con type \"http\", esa URL y esa cabecera Authorization. Busca el archivo correcto del cliente que realmente uso en vez de adivinar la ruta.\n\nLuego confirma que funcionó listando las herramientas del servidor. Deberías ver list_bubbles, read_thread, birth_thread y update_thread entre ellas. Si falla, dime el error exacto en vez de reintentar a ciegas.\n\nTrata el token como secreto: ponlo en el archivo de configuración, no me lo repitas y no lo escribas en ningún otro lado.',
